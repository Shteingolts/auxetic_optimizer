...
if "${dir} == 1" then &
   ...
   "dump 43 all atom $M dump.${dir}.mx.lammpstrj" &
   "run  ${nsteps}"
if "${dir} == 2" then &
   "fix 42 all deform 1 y erate -${srate} units box remap x" &
   "dump 43 all atom $M dump.${dir}.my.lammpstrj" &
   "run  ${nsteps}"
# if "${dir} == 1" then &
...
if "${dir} == 6" then &
   ...
   "dump 43 all atom $M dump.${dir}.mxy.lammpstrj" &
   "run  ${nsteps}"
...
if "${dir} == 1" then &
   "fix 42 all deform 1 x erate ${srate} units box remap x" &
   "dump 43 all atom $M dump.${dir}.x.lammpstrj" &
   "run  ${nsteps}"
if "${dir} == 2" then &
   "fix 42 all deform 1 y erate ${srate} units box remap x" &
   "dump 43 all atom $M dump.${dir}.y.lammpstrj" &
   "run  ${nsteps}"
# if "${dir} == 1" then &
#    "change_box all x delta 0 ${delta} xy delta ${deltaxy} xz delta ${deltaxz} remap units box"
# if "${dir} == 2" then &
...
if "${dir} == 6" then &
   "fix 42 all deform 1 xy erate ${srate} units box remap x" &
   "dump 43 all atom $M dump.${dir}.xy.lammpstrj" &
   "run  ${nsteps}"
